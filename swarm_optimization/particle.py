...
class Particle (object):
    ...
    def __init__ (self, swarm):
        ...
        if swarm.max_income == True and swarm.min_risk == False:
            self.__localBestFinalFunc = swarm.getFinalFunc (self.__currentPosition)[0]
        else:
            self.__localBestFinalFunc = swarm.getFinalFunc(self.__currentPosition)[1]
    ...
    def __getInitPosition (self, swarm):
        ...
        """
        return random.rand (swarm.dimension) * (swarm.maxvalues - swarm.minvalues) + swarm.minvalues
    ...
    def nextIteration (self, swarm):
        ...
        if swarm.max_income == True and swarm.min_risk == False:
            ...
            if (finalFunc[0] > self.__localBestFinalFunc):
                self.__localBestPosition = self.__currentPosition[:]
                self.__localBestFinalFunc = finalFunc[0]
        else:
            if (finalFunc[1] < self.__localBestFinalFunc):
                self.__localBestPosition = self.__currentPosition[:]
                self.__localBestFinalFunc = finalFunc[1]
